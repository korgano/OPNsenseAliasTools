...
logging.basicConfig(filename='download_ips-.log', level=logging.DEBUG,
...
def save_ips_to_file(ips, output_file):
    ...
    try:
        with open(output_file, 'w') as f:
            for ip in ips:
                # {IP} has space after it for CSV import
                f.write(f"{ip} ")
...
def main():
    urls = [
        'https://www.example.com/file1.txt', # example block list
        'https://www.example.com/file2.txt', # example block list
        # Add more URLs here
    ]

    ip_ranges_output_file = 'cleaned-ranges-.txt'
    ip_addresses_output_file = 'cleaned-IPs-.txt'
